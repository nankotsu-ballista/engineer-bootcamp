...
for i in range(q):
    ...
    if num==1:
        if r!=n:    
            tmp=lazy_segtree.prod(r-1,r)
            if tmp==1:
                lazy_segtree.apply(r-1,r,-1)
            if tmp==0:
                lazy_segtree.apply(r-1,r,1)
        if l==1:
            continue
        tmp=lazy_segtree.prod(l-2,l-1)
        if tmp==1:
            lazy_segtree.apply(l-2,l-1,-1)
        if tmp==0:
            lazy_segtree.apply(l-2,l-1,1)
    if num==2:
        if l==r:
