...
lazy_segtree2 = LazySegTree(op, e, mapping, composition,_id,lst2)

A=list(map(int,input().split()))
for i in A:
    lazy_segtree.apply(i,i+1,+i)
for i in A:
    lazy_segtree2.apply(i,i+1,+1)
    
for _ in range(q):
    num,l,r = map(int, input().split())
    if num==1:
        nn=A[l-1]
        lazy_segtree2.apply(nn,nn+1,-1)
        lazy_segtree.apply(nn,nn+1,-nn)
        A[l-1]=r
        lazy_segtree.apply(r,r+1,+r)
        lazy_segtree2.apply(r,r+1,+1)
    elif num==2:
        if l<r:
            #print(A)
            c1=(lazy_segtree.prod(l,r+1))
            c2=(l*lazy_segtree2.prod(0,l))
            c3=(r*lazy_segtree2.prod(r+1,5*10**5+1))
            print(c1+c2+c3)
        if r<=l:
            print(n*l)
            
# print(A)
# print(lazy_segtree2.prod(0,8))
